...
try {
  ...
  execSync(`npx prisma migrate dev --name ${migrationName} --create-only --config=${prismaDir}/prisma.config.ts`, {
    stdio: 'inherit',
    env: {
      ...process.env,
  ...
  console.log('💡 Next step: Review the migration file and then run: npm run db:migrate')
} catch (error) {
